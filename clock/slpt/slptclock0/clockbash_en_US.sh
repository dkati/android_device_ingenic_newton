...
slpt set_view_str ../setting/display/ clocks-to-show date-en-view\ time-view\ week-en-view #设置显示日期、时间、星期表盘
...
slpt set_pic_grp week_nums/ week_en_US/    #将当前目录下的week_en_US目录内的0-7数字图片数据 加载到week_nums中（显示星期几）
...
slpt set_view_int week-en-view start-x 0x3e start-y 0x92 #设置日期显示的位置

slpt set_view_int date-en-view start-x 0x75 start-y 0x92 #设置星期显示的位置
