...
slpt set_view_str ../setting/display/ clocks-to-show analog-clock-viewa\ date-cn-view\ week-cn-view #设置显示模拟以及日期、时间、星期表盘

slpt set_view_int analog-clock-viewa level 100 #设置表盘上下层关系 ，模拟表盘在上，数字表盘在下

slpt set_view_int date-cn-view level 1 #设置表盘上下层关系，模拟表盘在上，数字表盘在下

slpt set_view_int week-cn-view level 2 #设置表盘上下层关系，模拟表盘在上，数字表盘在下
...
slpt set_pic clock/week_name week_zh_CN.bmp #加载指定"星期“数据到表盘缓存clock/week_name中

slpt set_pic clock/day_name /system/slptclock/slptclock0/day.bmp #加载指定"星期“数据到表盘缓存clock/date_name中

slpt set_pic clock/mon_name /system/slptclock/slptclock0/month.bmp #加载指定"星期“数据到表盘缓存clock/date_name中

slpt set_pic_grp week_nums/ /system/slptclock/slptclock0/week_zh_CN/    #将当前目录下的week_en_US目录内的0-7数字图片数据 加载到week_nums中（显示星期几）
...
slpt set_view_int date-cn-view center-hor 1 start-y 180 #设置日期显示的位置

slpt set_view_int week-cn-view center-hor 1 start-y 155 #设置星期显示的位置
